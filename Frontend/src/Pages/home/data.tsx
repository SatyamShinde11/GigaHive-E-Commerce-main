import HomeImg1 from "../../assets/HomeImg/slider-1.webp";
import HomeImg2 from "../../assets/HomeImg/slider-2.webp";
import HomeImg3 from "../../assets/HomeImg/slider-3.webp";

import HomeImgMobile1 from "../../assets/HomeImg/mobile_slider_1.webp";
import HomeImgMobile2 from "../../assets/HomeImg/mobile_slider_2.webp";
import HomeImgMobile3 from "../../assets/HomeImg/mobile_slider_3.webp";

import Barnd1 from "../../assets/HomeImg/brand/logo1.webp";
import Barnd2 from "../../assets/HomeImg/brand/logo2.webp";
import Barnd3 from "../../assets/HomeImg/brand/logo3.webp";
import Barnd4 from "../../assets/HomeImg/brand/logo4.webp";
import Barnd5 from "../../assets/HomeImg/brand/logo5.webp";

export const slideData = [
  {
    id: 1,
    img: HomeImg1,
    ImgMobile: HomeImgMobile1,
    MobileTitle: `Summer Style Sensations.`,
    heading: (
      <>
        Summer Style <br /> Sensations.
      </>
    ),
    para: (
      <>
        Having plain clothing makes you look ordinary. We can assist you <br />{" "}
        in choosing the right dresses with Foesta.
      </>
    ),
    btn: "Shop now",
  },
  {
    id: 2,
    ImgMobile: HomeImgMobile2,
    MobileTitle: `Winter Style Delights.`,
    img: HomeImg2,
    heading: (
      <>
        Winter Style <br /> Delights.
      </>
    ),
    para: (
      <>
        Having plain clothing makes you look ordinary. We can assist you <br />{" "}
        in choosing the right dresses with Foesta.
      </>
    ),
    btn: "Shop now",
  },
  {
    id: 3,
    ImgMobile: HomeImgMobile3,
    img: HomeImg3,
    MobileTitle: `Spring Style Inspirations.`,
    heading: (
      <>
        Spring Style <br /> Inspirations.
      </>
    ),
    para: (
      <>
        Having plain clothing makes you look ordinary. We can assist you <br />{" "}
        in choosing the right dresses with Foesta.
      </>
    ),
    btn: "Shop now",
  },
];

export const ProductData = [
  {
    token:
      "Z2NwLXVzLWNlbnRyYWwxOjAxSlRaREhHNzc5UFYyQVBDWkhHMjVDUUU2?key=f7a5953e0270d6edff467f93984e1cfe",
    original_total_price: 84800,
    total_price: 84800,
    total_discount: 0,
    total_weight: 1500,
    item_count: 13,

    items: [
      {
        title: "Vintage-Inspired Floral Midi Dress",
        price: 7000,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/13_8d808684-401c-46d2-adfa-dfb619e65437.png?v=1716285430",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/12_c0ced60c-918f-484f-94db-415f4e609aae.png?v=1716285375",
        quantity: 1,
        discount_percent: 28,
        variant: "Black / Medium",
        color: "Black",
        size: "Medium",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A stylish black midi dress with vintage floral prints, perfect for any elegant occasion.",
      },
      {
        title: "Trendy Toddler Denim Jacket",
        price: 6500,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/12_c0ced60c-918f-484f-94db-415f4e609aae.png?v=1716285375",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/13_8d808684-401c-46d2-adfa-dfb619e65437.png?v=1716285430",
        quantity: 1,
        variant: "Default Title",
        color: "White",
        size: "Small",
        stock: "In stock",
        brand: "parthers-demo",
        description:
          "A cool and durable denim jacket designed especially for toddlers. Stylish and comfortable for daily wear.",
      },
      {
        title: "Trendy Denim Jacket with Patches",
        price: 7000,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/8_2ccc4c78-855a-4461-b9d6-f56a8669ee4c.png?v=1716285336",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/6_d1d65842-5cf5-44b3-b8a5-cc40d2815665.png?v=1716285291",
        quantity: 1,
        discount_percent: 28,
        variant: "Default Title",
        color: "Blue",
        size: "Large",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A bold denim jacket in blue featuring trendy patches for a modern, street-style look.",
      },
      {
        title: "Statement Geometric Print Scarf",
        price: 5000,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/6_d1d65842-5cf5-44b3-b8a5-cc40d2815665.png?v=1716285291",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/8_2ccc4c78-855a-4461-b9d6-f56a8669ee4c.png?v=1716285336",
        quantity: 1,
        variant: "Black / Small",
        color: "Black",
        size: "Small",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "Add a stylish twist to your outfit with this black scarf featuring a bold geometric print.",
      },
      {
        title: "Modern Slim Fit Suit",
        price: 7000,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/11.png?v=1716285223",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/2.png?v=1716285182",
        quantity: 1,
        variant: "Default Title",
        color: "Blue",
        size: "Medium",
        stock: "In stock",
        brand: "parthers-demo",
        description:
          "A sophisticated slim-fit suit in blue for the modern man. Perfect for business or formal events.",
      },
      {
        title: "Modern Slim Fit Chino Pants",
        price: 8400,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/2.png?v=1716285182",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/11.png?v=1716285223",
        quantity: 1,
        discount_percent: 28,
        variant: "Default Title",
        color: "Black",
        size: "Medium",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A sleek pair of black chino pants tailored for a perfect fit and versatile everyday wear.",
      },
      {
        title: "Stylish Quilted Winter Jacket",
        price: 7900,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/3.png?v=1716285176",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/5.png?v=1716285168",
        quantity: 1,
        variant: "Default Title",
        color: "Yellow",
        size: "Large",
        stock: "In stock",
        brand: "parthers-demo",
        description:
          "Keep warm in style with this bright yellow quilted jacket designed for chilly winters.",
      },
      {
        title: "Casual Cotton Blazer",
        price: 5200,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/5.png?v=1716285168",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/3.png?v=1716285176",
        quantity: 1,
        variant: "Default Title",
        color: "White",
        size: "Small",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A lightweight cotton blazer in white, ideal for casual or semi-formal outings.",
      },
      {
        title: "Trendy Checked Casual Shirt",
        price: 5300,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/7.png?v=1716285160",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/9.png?v=1716285153",
        quantity: 1,
        discount_percent: 28,
        variant: "Default Title",
        color: "Blue",
        size: "Medium",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A comfortable and stylish checked shirt in blue, great for casual wear.",
      },
      {
        title: "Elegant Embroidered Kurti",
        price: 6100,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/9.png?v=1716285153",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/7.png?v=1716285160",
        quantity: 1,
        variant: "Default Title",
        color: "Yellow",
        size: "Large",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "An elegant yellow kurti with beautiful embroidery work, perfect for festive occasions.",
      },
      {
        title: "Elegant Satin Saree with Blouse",
        price: 6100,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/10.png?v=1716285145",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/1.png?v=1716285122",
        discount_percent: 28,
        quantity: 1,
        variant: "Default Title",
        color: "Black",
        size: "Small",
        stock: "In stock",
        brand: "parthers-demo",
        description:
          "A graceful black satin saree paired with a matching blouse for an elegant traditional look.",
      },
      {
        title: "Chic Off-Shoulder Evening Gown",
        price: 6300,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/1.png?v=1716285122",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/10.png?v=1716285145",
        quantity: 1,
        variant: "Default Title",
        color: "White",
        size: "Large",
        stock: "In stock",
        brand: "Foesta-demo",
        description:
          "A stunning off-shoulder gown in white, designed for glamourous evening events.",
      },
      {
        title: "Cotton Printed Night Suit",
        price: 6000,
        image:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/4.png?v=1716285106",
        image2:
          "https://cdn.shopify.com/s/files/1/0638/6879/7087/files/1.png?v=1716285122",
        discount_percent: 28,
        quantity: 1,
        variant: "Default Title",
        color: "Blue",
        size: "Medium",
        stock: "In stock",
        brand: "parthers-demo",
        description:
          "Stay cozy and stylish with this blue cotton night suit featuring fun prints.",
      },
    ],
  },
];

export const ReviewsData = [
  {
    title: "Product Price!",
    review:
      "I'm so impressed with the level of customer service I received. I had an issue with my order, and the support was amazing.",
    name: "Esther Howard",
    role: "Designer",
  },
  {
    title: "Delivery Services!",
    review:
      "I had a great experience shopping on this website. The interface is user-friendly, making it easy to find products.",
    name: "Michel",
    role: "Customer",
  },
  {
    title: "Good Support!",
    review:
      "I'm so impressed with the level of customer service I received. I had an issue with my order, and the support was amazing.",
    name: "Jenny",
    role: "Dev",
  },
  {
    title: "Product Quality!",
    review:
      "I absolutely love shopping here! The selection is fantastic, the prices are competitive. Highly recommend.",
    name: "Wade Warren",
    role: "Customer",
  },
  {
    title: "Delivery Services!",
    review:
      "I had a great experience shopping on this website. The interface is user-friendly, making it easy to find products.",
    name: "Kristin Watson",
    role: "Customer",
  },
];

export const BrandImg = [
  { BrandImg: Barnd1 },
  { BrandImg: Barnd2 },
  { BrandImg: Barnd3 },
  { BrandImg: Barnd4 },
  { BrandImg: Barnd5 },
    { BrandImg: Barnd1 },
  { BrandImg: Barnd2 },
  { BrandImg: Barnd3 },
  { BrandImg: Barnd4 },
  { BrandImg: Barnd5 },
];
